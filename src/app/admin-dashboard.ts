...
import {isPlatformBrowser, NgClass, SlicePipe, DatePipe} from '@angular/common';
...
@Component({
  ...
  imports: [MatIconModule, MatRippleModule, MatMenuModule, MatButtonModule, MatBadgeModule, NgClass, SlicePipe, DatePipe, FormsModule, CdkDropList, CdkDrag, CdkDragHandle],
  template: `
      ...
      <main class="flex-grow flex flex-col relative h-full lg:h-screen overflow-hidden">
        ...
        <header class="flex items-center justify-between lg:justify-end px-4 sm:px-8 py-3 sm:py-4 bg-white/80 backdrop-blur-xl border-b border-slate-200 sticky top-0 z-10 w-full">
          ...
          <div class="flex items-center gap-2 sm:gap-4">
            <button mat-icon-button (click)="isRightSidebarOpen.set(!isRightSidebarOpen())" class="text-slate-500 hover:text-primary transition-all duration-300 transform hover:scale-110 active:scale-95 bg-slate-50 hover:bg-slate-100">
        ...
        >
          ...
          <div class="py-4 flex flex-col items-center justify-start gap-[2px] w-full">
            <!-- Notifications Info -->
            <button mat-icon-button class="text-orange-500 hover:bg-orange-50 transition-colors" title="Alerts">
              <mat-icon matBadge="3" matBadgeColor="warn" matBadgeSize="small">notifications_active</mat-icon>
            </button>
          ...
          <div #animateItem class="bg-white rounded-[2rem] overflow-hidden border border-slate-200/60 shadow-xs mb-8">
            <div class="p-6 sm:p-8 border-b border-slate-50 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
              ...
              <div class="flex items-center gap-4">
                <input type="text" [ngModel]="projectFilter()" (ngModelChange)="projectFilter.set($event)" placeholder="Search projects..." class="bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-primary/20 w-full sm:w-64">
                <button mat-flat-button (click)="createNewAdminProject()" class="!rounded-full !bg-primary !text-white !font-bold !px-6 !py-5 shadow-lg shadow-blue-500/20 hover:scale-105 transition-transform">
...
export class AdminDashboard implements AfterViewInit, OnInit {
  ...
  activeSection = signal<'dashboard'|'services'|'products'|'website'|'profile'|'settings'|'inquiries'>('dashboard');
